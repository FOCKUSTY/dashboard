...
export class UsersService {
  public async getUser(id: string): Promise<IResponse<IUser>> {
    try {
      ...
      if (!user) { return createError("user not found", null) };
  ...
  public async updateUser(id: string, user: Partial<IUser>): Promise<IResponse<UpdateWriteOpResult, UpdateWriteOpResult|null>> {
    try {
      const updatedUser = await User.updateOne({id}, { ...user });
      
      if (!updatedUser.acknowledged) { return createError("user not updated", updatedUser) }
  ...
  public async deleteUser(id: string): Promise<IResponse<DeleteResult, DeleteResult|null>> {
    try {
      const deletedUser = await User.deleteOne({id});

      if (!deletedUser.acknowledged) { return createError("user not deleted", deletedUser) };
